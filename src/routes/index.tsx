  ...
  head: () => ({
    meta: [
      { title: "Бушава Азбука — Играј и учи македонска азбука" },
      {
        name: "description",
        content:
          "Бушава Азбука: магичен свет на букви и игри за деца од 3 до 8 години. Играј, слушај и учи на македонски.",
      },
...
function Index() {
  ...
  const scrollToSection = (id: "home" | "games" | "rewards") => {
  ...
  return (
    <main className="min-h-screen overflow-x-hidden">
      {/* NAV */}
      <header className="relative px-4 sm:px-8 py-5 flex items-center justify-end max-w-7xl mx-auto">
        <nav className="hidden sm:flex items-center gap-2 absolute left-1/2 -translate-x-1/2">
          {[
            { icon: Home, label: "Дома", sectionId: "home" as const },
            { icon: Gamepad2, label: "Игри", sectionId: "games" as const },
            { icon: Trophy, label: "Награди", sectionId: "rewards" as const },
          ].map((n) => (
            <button
              key={n.label}
              onClick={() => scrollToSection(n.sectionId)}
              className="flex items-center gap-2 px-5 py-3 rounded-2xl font-semibold text-base bg-white/70 hover:bg-white btn-pop"
              style={{ minHeight: 56 }}
            >
        ...
        <button
          ...
          onClick={() =>
            speak("Добредојде во Бушавата Азбука! Притисни го големото копче за да играш.")
          }
      ...
      <section
        id="home"
        className="relative max-w-7xl mx-auto px-4 sm:px-8 pt-4 pb-20 scroll-mt-24"
      >
        ...
        <div className="relative grid lg:grid-cols-2 gap-8 items-center">
          <div className="order-1 lg:order-1 text-center lg:text-left">
            ...
            <h1
              className="font-display font-extrabold text-5xl sm:text-6xl lg:text-7xl leading-[0.95]"
              style={{ color: "var(--bukvozhder-blue)" }}
            >
              Добре дојде во
              <br />
      ...
      <section className="max-w-5xl mx-auto px-4 sm:px-8 -mt-6 mb-16">
        <div className="rounded-3xl p-6 sm:p-8 card-pop bg-white">
          <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
            ...
            <span className="text-lg font-bold" style={{ color: "var(--bushava-orange)" }}>
              {stars} / 12 ѕвезди
            </span>
          </div>
          <div className="h-6 rounded-full bg-foreground/10 overflow-hidden">
            <div
              ...
              style={{
                width: `${(stars / 12) * 100}%`,
                background:
                  "linear-gradient(90deg, var(--cheerful-yellow), var(--bushava-orange), var(--warm-red))",
              }}
            />
          </div>
          <div className="flex justify-between mt-4 gap-2 flex-wrap">
            {Array.from({ length: 12 }).map((_, i) => (
              <Star
                ...
                style={{
                  color: i < stars ? "var(--cheerful-yellow)" : "var(--foreground)",
      ...
      {/* GAMES */}
      <section id="games" className="max-w-7xl mx-auto px-4 sm:px-8 mb-20 scroll-mt-24">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
          {games.map((g) => (
            ...
            >
              ...
              <h3
                className="font-display font-extrabold text-xl sm:text-2xl relative"
                style={{ color: g.color }}
              >
                {g.title}
              </h3>
      ...
      {/* DASHBOARD / BADGES */}
      <section id="rewards" className="max-w-7xl mx-auto px-4 sm:px-8 mb-24 scroll-mt-24">
        <div
          className="rounded-[2rem] p-6 sm:p-10 card-pop"
          style={{
            background: "linear-gradient(135deg, var(--bukvozhder-blue), oklch(0.5 0.18 250))",
          }}
        >
          ...
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-4">
            {[
              ...
              <div
                key={b.n}
                className={`rounded-2xl p-4 text-center ${b.got ? "bg-white" : "bg-white/15"}`}
              >
                <div className={`text-4xl sm:text-5xl mb-1 ${b.got ? "" : "grayscale opacity-50"}`}>
                  {b.e}
                </div>
                <div
                  className={`text-xs sm:text-sm font-bold ${b.got ? "text-foreground" : "text-white/70"}`}
                >
                  {b.n}
                </div>
              </div>
